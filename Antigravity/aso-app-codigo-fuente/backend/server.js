                ...
                entries.forEach(entry => {
                    ...
                    try {
                        ...
                        if (stats.isDirectory()) {
                            ...
                            skills.push({...parseSkillFile(skillFilePath, entry, true), scope: 'project'});
                        } else if (entry.endsWith('.md')) {
                            // Soporte para archivos .md sueltos como skills
                            const skillId = entry.replace('.md', '');
                            skills.push({...parseSkillFile(entryPath, skillId, true), scope: 'project'});
                        }
                    } catch (e) {
                        console.error(`Error reading skill entry ${entry}:`, e);
                    }
                });
            } catch (e) {
                console.error("Error reading project skills directory:", e);
            }
        }
    }
...
// Endpoint para Memoria de Sesión
app.get('/api/session-memory', (req, res) => {
    if (!PROJECT_PATH) {
        return res.json({ success: false, message: "No hay un proyecto activo." });
    }

    const memoryPath = path.join(PROJECT_PATH, 'directivas', 'memoria_sesion.md');
    
    if (!fs.existsSync(memoryPath)) {
        return res.json({ success: false, message: `Archivo de memoria no encontrado en: ${memoryPath}` });
    }
...
app.post('/api/session-memory', (req, res) => {
    if (!PROJECT_PATH) {
        return res.json({ success: false, message: "No hay un proyecto activo." });
    }
    ...
    try {
        ...
        res.json({ success: true, message: "Memoria de sesión actualizada correctamente." });
    } catch (e) {
