...
function App() {
  ...
  const [activeTab, setActiveTab] = useState('memory'); // 'memory' | 'activas' | 'vault' | 'factory' - Por defecto 'memory' para Cero Amnesia
  ...
  const saveSessionMemory = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/session-memory`, {
        ...
        body: JSON.stringify({ content: editedMemory })
      });
      const data = await res.json();
      if (data.success) {
  ...
  useEffect(() => {
    fetchSessionMemory();
    const interval = setInterval(fetchSessionMemory, 30000);
  ...
  const tabSkills = skills.filter(s => activeTab === 'activas' ? s.isActive : !s.isActive);
  ...
  return (
    <div className="flex h-screen bg-[#0e1320] text-[#dee2f5] overflow-hidden font-sans selection:bg-[#00fbfb]/30">
        ...
        <nav className="flex-1 px-4 mt-8 space-y-2">
          ...
          <div className="pt-4 mt-4 border-t border-[#3a4a49]/30">
             ...
             <button 
              ...
              className={`w-full flex items-center gap-2 px-4 py-3 mt-2 rounded-md transition-all ${activeTab === 'memory' ? 'bg-[#007070]/30 border border-[#00fbfb]/50 neon-text' : 'hover:bg-[#161b29] text-[#b9cac9]'}`}
             >
               <span className="text-lg">🧠</span>
               <span className="font-medium mr-auto">Memoria de Sesión</span>
               <div className="w-2 h-2 rounded-full bg-[#00fbfb] animate-pulse shadow-[0_0_8px_#00fbfb]"></div>
             </button>
          </div>
        </nav>
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <header className="h-20 flex items-center justify-between px-8 bg-[#0e1320]/80 backdrop-blur-md z-10 border-b border-[#3a4a49]/30 shrink-0">
          <div className="flex items-center gap-4">
            <h2 className="text-xl font-medium text-white flex items-center gap-3">
              {activeTab === 'activas' ? 'Skills Activas' : activeTab === 'vault' ? 'Skills Guardadas' : activeTab === 'memory' ? 'Memoria de Sesión' : 'Skill Factory (Co-Pilot)'}
          ...
          {activeTab !== 'factory' && activeTab !== 'memory' && (
          ...
          <div className="flex items-center gap-3">
             <button
               ...
               className={`p-2 rounded-lg border transition-all flex items-center gap-2 ${showRightPanel ? 'bg-[#007070]/20 border-[#00fbfb] text-[#00fbfb]' : 'bg-[#161b29] border-[#3a4a49]/50 text-[#839493] hover:text-white'}`}
               title="Ver Memoria Persistente"
             >
                <span className="text-lg">🧠</span>
                <span className="text-xs font-medium hidden md:block">MEMORIA</span>
             </button>
          </div>
        </header>

        {/* Content Area */}
        {(activeTab === 'activas' || activeTab === 'vault') ? (
        <div className="flex-1 overflow-y-auto p-8 z-0">
              ...
              {allCategories.map(cat => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-mono tracking-wider transition-all border ${
                    activeCategory === cat
                      ? 'bg-[#00fbfb]/20 border-[#00fbfb] text-[#00fbfb] shadow-[0_0_10px_rgba(0,251,251,0.3)]'
                      : 'bg-[#090e1b] border-[#3a4a49]/50 text-[#839493] hover:border-[#00fbfb]/40 hover:text-[#b9cac9]'
                  }`}
                >
                  {cat}
          ...
          <div className="flex-1 overflow-y-auto p-8 z-0 skill-md-viewer relative">
            {loadingMemory && !sessionMemory ? (
              <div className="flex items-center justify-center h-full">
                <div className="text-[#00fbfb] animate-pulse">Cargando memoria de sesión...</div>
              </div>
            ) : (
              <div className="max-w-4xl mx-auto">
                <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center gap-4">
                     <div className="w-12 h-12 rounded-2xl bg-[#00fbfb]/10 border border-[#00fbfb]/30 flex items-center justify-center text-2xl shadow-[0_0_20px_rgba(0,251,251,0.1)]">🧠</div>
                     <div>
                       <h1 className="text-3xl font-bold text-white tracking-tight">Memoria de Sesión</h1>
                       <p className="text-[#839493] text-sm font-mono mt-1 uppercase tracking-widest">Protocolo Cero Amnesia Activo</p>
                     </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    {isEditingMemory ? (
                      <>
                        <button 
                          onClick={() => { setIsEditingMemory(false); setEditedMemory(sessionMemory); }}
                          className="px-4 py-2 border border-[#3a4a49] text-[#839493] hover:text-white rounded-lg transition-all text-xs"
                        >
                          Cancelar
                        </button>
                        <button 
                          onClick={saveSessionMemory}
                          className="px-4 py-2 bg-[#00fbfb] text-black font-semibold rounded-lg transition-all text-xs hover:shadow-[0_0_15px_#00fbfb]"
                        >
                      ...
                      <button 
                        onClick={() => { setIsEditingMemory(true); setEditedMemory(sessionMemory); }}
                        className="px-4 py-2 bg-[#1b152d] border border-[#9b51e0]/30 text-[#d4b3ff] hover:border-[#9b51e0] rounded-lg transition-all text-xs flex items-center gap-2"
                      >
                         <span>✏️</span> Editar Manualmente
                      </button>
                    )}
                  </div>
                </div>

                <div className="glass-panel p-10 rounded-3xl border border-[#3a4a49]/30 shadow-2xl relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-2 h-full bg-[#00fbfb]/50"></div>
                  
                  {isEditingMemory ? (
                    <textarea
                      value={editedMemory}
                      onChange={(e) => setEditedMemory(e.target.value)}
                      className="w-full h-[60vh] bg-[#060a14] border border-[#1e2d2c] rounded-xl p-8 text-sm font-mono text-[#b9cac9] focus:outline-none focus:border-[#00fbfb]/50 leading-relaxed resize-none"
                    />
                  ) : (
                    <ReactMarkdown
                      remarkPlugins={[remarkGfm]}
                      components={{
                        h1: ({children}) => <h1 className="text-3xl font-bold text-white mb-6 border-b border-[#3a4a49]/50 pb-4">{children}</h1>,
                        h2: ({children}) => <h2 className="text-xl font-semibold text-[#00fbfb] mb-4 mt-10 flex items-center gap-2"><span className="w-1.5 h-6 bg-gradient-to-b from-[#00fbfb] to-transparent rounded-full inline-block"></span>{children}</h2>,
                        h3: ({children}) => <h3 className="text-lg font-semibold text-[#b9cac9] mb-3 mt-8">{children}</h3>,
                        p: ({children}) => <p className="text-[#9db3b2] text-base leading-relaxed mb-5">{children}</p>,
                        ul: ({children}) => <ul className="space-y-3 mb-6 ml-1">{children}</ul>,
                        li: ({children}) => {
                          const content = children[0] === ' ' ? children.slice(1) : children;
                          return <li className="text-[#9db3b2] text-sm flex gap-4 bg-[#161b29]/30 p-3 rounded-xl border border-[#3a4a49]/20 hover:border-[#00fbfb]/30 transition-all"><span className="text-[#00fbfb] shrink-0 font-bold">●</span><span>{children}</span></li>;
                        },
                        code: ({inline, children}) => inline
                          ? <code className="text-[#00fbfb] bg-[#00fbfb]/10 border border-[#00fbfb]/20 px-1.5 py-0.5 rounded text-xs font-mono">{children}</code>
                          : <code className="block bg-[#060d1a] border border-[#1e2d2c] rounded-2xl p-8 text-xs font-mono text-[#b9cac9] overflow-x-auto leading-relaxed whitespace-pre shadow-inner my-6">{children}</code>,
                        blockquote: ({children}) => <blockquote className="border-l-4 border-[#00fbfb]/40 pl-8 my-8 text-[#7a9a99] italic bg-[#00fbfb]/5 py-6 rounded-r-2xl font-serif text-lg">{children}</blockquote>,
                  ...
                  <div className="mt-12 pt-8 border-t border-[#3a4a49]/30 flex items-center justify-between text-xs text-[#5c6c6b]">
                     <div className="flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-[#00fbfb] shadow-[0_0_5px_#00fbfb]"></span>
                        Sincronización en tiempo real activa
                     </div>
                     <button 
                      onClick={fetchSessionMemory}
                      className="hover:text-[#00fbfb] transition-all flex items-center gap-1"
                     >
                       <svg className={`w-3 h-3 ${loadingMemory ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg>
                       Refrescar ahora
                     </button>
                  </div>
                </div>
        ...
        {inspectModal.isOpen && (() => {
          ...
          return (
            ...
            >
              <div className="bg-[#0b1120] border border-[#1e2d2c] rounded-t-2xl md:rounded-2xl shadow-[0_0_60px_rgba(0,251,251,0.08)] flex flex-col w-full max-w-3xl h-[90vh] md:h-[85vh] relative overflow-hidden">

                {/* Header */}
                <div className="flex items-center gap-4 px-6 py-4 border-b border-[#1e2d2c]/80 bg-[#070b14] shrink-0">
                  <div className="w-10 h-10 rounded-lg bg-[#0e1a2c] border border-[#1e2d2c] flex items-center justify-center text-xl shrink-0">📄</div>
                  <div className="flex-1 min-w-0">
                    <h2 className="text-white font-semibold text-lg leading-tight truncate">{inspectModal.skillName}</h2>
                    <div className="flex items-center gap-2 mt-0.5">
                      {sk.category && (
                        <span className="text-[10px] font-mono text-[#00c8c8] bg-[#00fbfb]/10 border border-[#00fbfb]/20 px-2 py-0.5 rounded-full">{sk.category}</span>
                      )}
                      <span className={`text-[10px] font-mono px-2 py-0.5 rounded-full border ${sk.isActive ? 'text-[#00fbfb] bg-[#00fbfb]/10 border-[#00fbfb]/20' : 'text-[#839493] bg-[#839493]/10 border-[#839493]/20'}`}>
                  ...
                  <button
                    ...
                    className="ml-2 w-8 h-8 rounded-lg hover:bg-[#161b29] text-[#839493] hover:text-white transition-colors flex items-center justify-center shrink-0"
                  >
                ...
                <div className="flex-1 overflow-y-auto px-8 py-7 skill-md-viewer">
                  {inspectModal.loading ? (
                    <div className="flex items-center justify-center h-full">
                      <div className="text-[#00fbfb] animate-pulse font-mono text-sm">Cargando skill...</div>
                    </div>
                  ) : (
                    <ReactMarkdown
                      remarkPlugins={[remarkGfm]}
                      components={{
                        h1: ({children}) => <h1 className="text-2xl font-bold text-white mb-4 mt-2 pb-2 border-b border-[#1e2d2c]">{children}</h1>,
                        h2: ({children}) => <h2 className="text-lg font-semibold text-[#00fbfb] mb-3 mt-6 flex items-center gap-2"><span className="w-1 h-4 bg-[#00fbfb] rounded-full inline-block"></span>{children}</h2>,
                        h3: ({children}) => <h3 className="text-base font-semibold text-[#b9cac9] mb-2 mt-4">{children}</h3>,
                        h4: ({children}) => <h4 className="text-sm font-semibold text-[#7a9a99] mb-1 mt-3 uppercase tracking-wider">{children}</h4>,
                        p: ({children}) => <p className="text-[#9db3b2] text-sm leading-relaxed mb-3">{children}</p>,
                        ...
                        li: ({children}) => <li className="text-[#9db3b2] text-sm leading-relaxed flex gap-2"><span className="text-[#00fbfb] mt-1 shrink-0">›</span><span>{children}</span></li>,
                        code: ({inline, children}) => inline
                          ? <code className="text-[#00fbfb] bg-[#00fbfb]/10 border border-[#00fbfb]/20 px-1.5 py-0.5 rounded text-xs font-mono">{children}</code>
                          : <code className="block bg-[#060d1a] border border-[#1e2d2c] rounded-lg p-4 text-xs font-mono text-[#b9cac9] overflow-x-auto leading-relaxed whitespace-pre">{children}</code>,
                        pre: ({children}) => <div className="mb-4">{children}</div>,
                        blockquote: ({children}) => <blockquote className="border-l-2 border-[#00fbfb]/40 pl-4 my-3 text-[#7a9a99] italic text-sm">{children}</blockquote>,
                        strong: ({children}) => <strong className="text-white font-semibold">{children}</strong>,
                        em: ({children}) => <em className="text-[#b9cac9] italic">{children}</em>,
                        hr: () => <hr className="border-[#1e2d2c] my-6" />,
                        a: ({href, children}) => <a href={href} target="_blank" rel="noopener noreferrer" className="text-[#00fbfb] hover:underline">{children}</a>,
                        table: ({children}) => <div className="overflow-x-auto mb-4"><table className="w-full text-sm border-collapse">{children}</table></div>,
                        th: ({children}) => <th className="text-left text-xs font-mono text-[#00fbfb] uppercase tracking-wider px-3 py-2 border-b border-[#1e2d2c] bg-[#060d1a]">{children}</th>,
                        td: ({children}) => <td className="px-3 py-2 text-[#9db3b2] border-b border-[#1e2d2c]/50 text-sm">{children}</td>,
                        tr: ({children}) => <tr className="hover:bg-[#00fbfb]/5 transition-colors">{children}</tr>,
                      }}
                    >
                      {strippedContent}
                    </ReactMarkdown>
        ...
        {/* Right Sidebar - Persistent Memory Viewer */}
        <aside 
          className={`glass-panel border-l border-[#3a4a49]/30 flex flex-col transition-all duration-300 ease-in-out relative z-20 ${showRightPanel ? 'w-96' : 'w-0 overflow-hidden border-none'}`}
        >
          {showRightPanel && (
            <>
              <div className="p-6 border-b border-[#3a4a49]/30 flex items-center justify-between bg-[#0e1320]/50 sticky top-0 z-10">
                 <div className="flex items-center gap-3">
                   <span className="text-xl">🧠</span>
                   <h3 className="font-semibold text-white">PROGRESO LIVE</h3>
                 </div>
                 <button onClick={() => setShowRightPanel(false)} className="text-[#839493] hover:text-white p-1">
                   <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                 </button>
              </div>
              
              <div className="flex-1 overflow-y-auto p-6 skill-md-viewer bg-gradient-to-b from-transparent to-[#00fbfb]/5">
                <ReactMarkdown
                  remarkPlugins={[remarkGfm]}
                  components={{
                    h1: ({children}) => <h1 className="text-xl font-bold text-white mb-4 border-b border-[#3a4a49]/50 pb-2">{children}</h1>,
                    h2: ({children}) => <h2 className="text-sm font-bold text-[#00fbfb] mb-2 mt-6 uppercase tracking-wider">{children}</h2>,
                    p: ({children}) => <p className="text-[#9db3b2] text-xs leading-relaxed mb-3">{children}</p>,
                    ul: ({children}) => <ul className="space-y-2 mb-4 ml-1">{children}</ul>,
                    li: ({children}) => <li className="text-[#9db3b2] text-[11px] leading-relaxed flex gap-2"><span className="text-[#00fbfb] shrink-0">›</span><span>{children}</span></li>,
                    code: ({children}) => <code className="text-[#00fbfb] bg-[#00fbfb]/10 px-1 rounded text-[10px] font-mono">{children}</code>,
                    strong: ({children}) => <strong className="text-white font-semibold">{children}</strong>,
                  }}
                >
                  {sessionMemory}
              ...
              <div className="p-4 border-t border-[#3a4a49]/30 text-[10px] text-center text-[#5c6c6b] font-mono tracking-tighter">
                AUTO-SYNC AT {new Date().toLocaleTimeString()}
              </div>
            </>
          )}
        </aside>
