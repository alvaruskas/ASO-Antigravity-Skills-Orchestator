...
| `⚡ WordPress` | **wp-performance** | Investigación y mejora del rendimiento de WordPress (agente backend): perfilado y medición (WP-CLI profile/doctor, Server-Timing, Query Monitor via cabeceras REST), optimización de bases de datos/consultas, opciones de carga automática, caché de objetos, cron, llamadas a la API HTTP y verificación segura. |
| `⚡ WordPress` | **wp-phpstan** | Configuración, ejecución o corrección del análisis estático PHPStan en proyectos WordPress (plugins/temas/sitios): configuración de phpstan.neon, baselines, tipado específico de WordPress y manejo de clases de plugins de terceros. |
| `⚡ WordPress` | **wp-playground** | Uso de WordPress Playground (WASM): creación de blueprints, configuración de sitios efímeros, prueba de plugins/temas e integración de Playground mediante CLI o navegador. |
| `⚡ WordPress` | **wp-plugin-development** | Desarrollo o depuración de plugins de WordPress: arquitectura, hooks (acciones/filtros), Settings API, mejores prácticas de seguridad, gestión de datos y ciclo de vida del plugin. |
